...
# Load audio files
audio_files = {
    "drowsy": r"/Users/sandra/Documents/final/alarm.mp3",
    "safe": r"/Users/sandra/Documents/final/alarm.mp3"
}

# Load YOLOv5 model
model = torch.hub.load("yolov5", 'custom', r"/Users/sandra/Documents/final/best (2).pt", source='local')

# Initialize MediaPipe Hand Tracker
mp_hands = mp.solutions.hands
hands = mp_hands.Hands()

# Dictionary mapping class numbers to their corresponding names
class_names = {
    0: "Driver_Wake",
    1: "Driver_Drowsy",
    2: "Steering",
...
def update_frame():
    ...
    if not ret:
        print("Error: Unable to read frame from the video file")
        return
    ...
    for idx, (bbox, cls, score) in enumerate(zip(bboxes, classes, scores)):
        x_min, y_min, x_max, y_max = bbox
        label = f"{class_names[cls]}: {score:.2f}"  # Use class name instead of number

        # Define text color based on class
        if cls == 1:  # Example: Driver_Drowsy
            text_color = (0, 0, 255)  # Red color for drowsy driver
        else:
            text_color = (255,0,0)  # White color for other classes

        # Draw bounding box and label
        cv2.rectangle(frame, (int(x_min), int(y_min)), (int(x_max), int(y_max)), (0, 255, 0), 2)
        cv2.putText(frame, label, (int(x_min), int(y_min - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 2)
...
unsafe_label = tk.Label(root, text="Hand On Steering Wheel: Unsafe", fg="red", font=("Helvetica", 14, "bold"))
unsafe_label.pack()
...
# Open the video file
cap = cv2.VideoCapture(r"/Users/sandra/Documents/final/video (2).mp4")
